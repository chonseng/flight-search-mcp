...
from flight_scraper.utils import (
    parse_duration, parse_price, parse_stops, format_date_for_input,
    random_delay, wait_for_element, safe_click, safe_fill, safe_get_text,
    retry_async_operation, setup_logging, RobustSelector, SelectorHealthMonitor,
    robust_find_element, robust_click, robust_fill, robust_get_text,
    ROBUST_SELECTOR_CONFIGS
)
from flight_scraper.core.models import (
    SelectorStrategy, SelectorFailureType, SelectorMonitoring
)
...
class TestBasicUtilities:
    ...
    def test_parse_stops(self):
        ...
        assert parse_stops("Non-stop flight") == 1  # Returns 1 because it finds the number "1" in "Non-stop"
...
class TestAsyncUtilities:
    ...
    async def test_retry_async_operation_no_exponential_backoff(self):
        ...
        with pytest.raises(Exception):
            await retry_async_operation(test_operation, max_attempts=3, delay=0.01, exponential_backoff=False)
...
class TestRobustSelector:
    ...
    async def test_find_element_fallback_to_structural(self):
        ...
        config = {
            "semantic": [".semantic-selector"],
            "structural": [".structural-selector"]
        }
    ...
    def test_categorize_failure_types(self):
        ...
        assert self.selector._categorize_failure(interaction_error) == SelectorFailureType.UNINTERACTABLE
        
        # Test visibility error
        visibility_error = Exception("not visible")
        assert self.selector._categorize_failure(visibility_error) == SelectorFailureType.UNINTERACTABLE
        
        # Test enabled error
        enabled_error = Exception("not enabled")
        assert self.selector._categorize_failure(enabled_error) == SelectorFailureType.UNINTERACTABLE
        ...
        assert self.selector._categorize_failure(detached_error) == SelectorFailureType.STALE_ELEMENT
        
        # Test permission error
        permission_error = Exception("permission denied")
        assert self.selector._categorize_failure(permission_error) == SelectorFailureType.PERMISSION_DENIED
        
        # Test generic error
        generic_error = Exception("something else happened")
        assert self.selector._categorize_failure(generic_error) == SelectorFailureType.STRUCTURE_CHANGED
...
class TestSelectorHealthMonitor:
    ...
    def test_get_health_report_with_recommendations(self):
        ...
        assert any("updating selector configurations" in rec.lower() for rec in report["recommendations"])
...
class TestSelectorConfigs:
    ...
    def test_robust_selector_configs_exist(self):
        ...
        expected_elements = [
            "origin_input", "destination_input", "departure_date",
            "return_date", "search_button", "flight_results"
        ]
    ...
    def test_setup_logging(self):
        ...
        with patch('flight_scraper.utils.logger') as mock_logger:
            with patch('flight_scraper.core.config.LOG_CONFIG', {
                "file": "test.log",
                "level": "INFO",
                "format": "{message}",
                "rotation": "1 MB",
