...
async def retry_async_operation(
    operation, max_attempts: int = 3, delay: float = 1.0, exponential_backoff: bool = True
) -> Any:
...
class RobustSelector:
    ...
    async def find_element(
        self, selector_config: Dict[str, List[str]], timeout: int = 10000
            ...
            for selector in selectors:
                ...
                try:
                    ...
                    if element:
                        ...
                        logger.info(
                            f"✅ Found {self.element_type} using {strategy.value} strategy: {selector}"
                        )
                        return element
                    ...
                    self._record_attempt(
                        selector, strategy, False, failure_type, str(e), attempt_time, dom_context
                    )
                    logger.debug(
                        f"❌ Failed {self.element_type} with {strategy.value}: {selector} - {str(e)}"
                    )
        ...
        logger.error(
            f"🚨 All selector strategies failed for {self.element_type} (took {total_time:.2f}s)"
        )
        return None
        ...
        try:
            ...
            if element:
                ...
                if is_visible and is_enabled:
                    ...
                    raise Exception(
                        f"Element found but not interactable (visible: {is_visible}, enabled: {is_enabled})"
                    )
    ...
    def _categorize_failure(self, error: Exception) -> SelectorFailureType:
        ...
        if "timeout" in error_str or "not found" in error_str:
            ...
            "not interactable" in error_str
            or "not visible" in error_str
    ...
    def _record_attempt(
        ...
        success: bool,
        failure_type: Optional[SelectorFailureType],
...
class SelectorHealthMonitor:
    ...
    def record_page_health(self, page_type: str, selector_monitors: Dict[str, SelectorMonitoring]):
        ...
        health = PageSelectorHealth(page_type=page_type, selector_monitoring=selector_monitors)
        ...
        health.overall_success_rate = (
            successful_selectors / total_selectors if total_selectors > 0 else 0.0
        )
    ...
    def _detect_structure_changes(self, monitors: Dict[str, SelectorMonitoring]) -> bool:
        ...
        for monitor in monitors.values():
            ...
            failure_types = [
                attempt.failure_type for attempt in monitor.attempts if not attempt.success
            ]
    ...
    def _generate_alerts(self, page_type: str, health: PageSelectorHealth):
        ...
        for element_type in health.critical_failures:
            ...
            alert = SelectorFailureAlert(
                ...
                failure_patterns=[
                    attempt.error_message for attempt in monitor.attempts if attempt.error_message
                ],
                recommended_actions=[
                ...
                first_failure=(
                    monitor.attempts[0].attempted_at
                    if monitor.attempts
                    else health.scraping_timestamp
                ),
                last_failure=(
                    monitor.attempts[-1].attempted_at
                    if monitor.attempts
                    else health.scraping_timestamp
                ),
                dom_changes_detected=any(
                    attempt.failure_type == SelectorFailureType.STRUCTURE_CHANGED
        ...
        for alert in alerts:
            if alert.severity == "critical":
                logger.error(
                    f"🚨 CRITICAL SELECTOR ALERT: {alert.element_type} - {alert.failure_patterns}"
                )
            else:
                logger.warning(
                    f"⚠️ SELECTOR WARNING: {alert.element_type} - {alert.failure_patterns}"
                )
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        if all_success_rates:
            report["overall_health"]["average_success_rate"] = sum(all_success_rates) / len(
                all_success_rates
            )
        ...
        for page_type, health in self.page_health.items():
            ...
            if health.overall_success_rate < 0.3:
                report["critical_issues"].append(
                    f"Very low success rate on {page_type}: {health.overall_success_rate:.1%}"
                )
...
ROBUST_SELECTOR_CONFIGS = {
    "origin_input": {
        ...
        "content_based": ['input:below(text("From"))', 'input:near(text("Origin"))'],
    },
    "destination_input": {
        "semantic": [
        ...
        "content_based": ['input:below(text("To"))', 'input:near(text("Destination"))'],
    },
    "departure_date": {
        "semantic": [
        ...
        "structural": [".date-picker input:first-child", ".date-input:first-of-type input"],
        "class_based": [".departure-date input", ".date-selector input"],
        "content_based": ['input:near(text("Departure"))', 'input[value*="-"]'],
    },
    "return_date": {
        "semantic": [
        ...
        "structural": [".date-picker input:nth-child(2)", ".date-input:nth-of-type(2) input"],
        "class_based": [".return-date input", ".date-selector input:nth-child(2)"],
        "content_based": ['input:near(text("Return"))', 'input:below(text("Return"))'],
    },
    "search_button": {
        "semantic": [
        ...
        "structural": ["form button", ".search-form button"],
        "class_based": [".search-button", ".submit-button", ".btn-search"],
        "content_based": [
    ...
    "flight_results": {
        "semantic": ['[data-testid*="flight"]', '[role="listitem"]', '[aria-label*="flight"]'],
        "structural": ['div[role="tabpanel"] ul li', ".results-container li", ".flight-list li"],
        "class_based": [".pIav2d", ".flight-result", ".flight-card"],
        "content_based": ['li:has-text("$")', 'div:has-text("hr"):has-text("$")'],
    },
}


async def robust_find_element(
    page: Page, element_type: str, timeout: int = 10000
) -> Optional[ElementHandle]:
    """
