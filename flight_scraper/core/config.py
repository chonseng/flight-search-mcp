...
class ScraperConfig(BaseSettings):
    ...
    wait_for_results: PositiveInt = Field(
        default=10000, description="Wait time for flight results in ms"
    )

    # Retry and delay settings
    retry_attempts: PositiveInt = Field(default=3, description="Number of retry attempts")
    min_delay: PositiveFloat = Field(
        default=2.0, description="Minimum delay between actions in seconds"
    )
    max_delay: PositiveFloat = Field(
        default=5.0, description="Maximum delay between actions in seconds"
    )
    ...
    model_config = ConfigDict(env_prefix="FLIGHT_SCRAPER_", case_sensitive=False)
...
class GoogleFlightsConfig(BaseSettings):
    ...
    model_config = ConfigDict(env_prefix="GOOGLE_FLIGHTS_", case_sensitive=False)
...
class SelectorConfig(BaseSettings):
    ...
    flight_containers: List[str] = Field(
        default=['div[role="tabpanel"] ul', '[data-testid="flight-offer"]', ".pIav2d", ".Rk10dc"],
        description="Selectors for flight result containers",
    )

    airline_name: List[str] = Field(
        default=[".Ir0Voe", ".sSHqwe", '[data-testid*="airline"]', 'img[alt*="logo"]'],
    ...
    duration: List[str] = Field(
        default=[".gvkrdb", ".AdWm1c", '[data-testid*="duration"]'],
        description="Selectors for flight duration",
    )

    stops: List[str] = Field(
        default=[".EfT7Ae .ogfYpf", ".c8rWCd", '[data-testid*="stops"]'],
        description="Selectors for number of stops",
    )

    price: List[str] = Field(
        default=[".f8F1md .YMlIz", ".U3gSDe", '[data-testid*="price"]', '[aria-label*="dollar"]'],
        description="Selectors for flight price",
    )

    model_config = ConfigDict(env_prefix="SELECTORS_", case_sensitive=False)
...
class LoggingConfig(BaseSettings):
    ...
    format: str = Field(
        default="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", description="Log message format"
    )
    ...
    model_config = ConfigDict(env_prefix="LOGGING_", case_sensitive=False)
...
class OutputConfig(BaseSettings):
    ...
    model_config = ConfigDict(env_prefix="OUTPUT_", case_sensitive=False)
...
class MCPConfig(BaseSettings):
    ...
    max_results_limit: PositiveInt = Field(
        default=50, description="Maximum results limit for MCP requests"
    )

    model_config = ConfigDict(env_prefix="MCP_", case_sensitive=False)
