...
from .models import FlightOffer, FlightSegment, SearchCriteria, ScrapingError
...
class DataExtractor:
    ...
    async def _find_flight_containers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for ul_data in ul_info['ulInfo']:
                logger.info(f"🔍 UL {ul_data['index'] + 1}: {ul_data['liCount']} li elements, "
            ...
            if ul_info['count'] >= 1:
                for ul_data in ul_info['ulInfo']:
                    if ul_data['liCount'] > 0:  # Only include ULs with flight data
                        ...
                        logger.info(f"🎯 Will extract from UL {ul_data['index'] + 1} "
    ...
    async def _extract_flight_elements(self, containers: List[Dict[str, Any]], 
                                     max_results: int) -> List[ElementHandle]:
        """
        ...
        for container in containers:
            try:
                ...
                if ul_data['found'] and ul_data['liCount'] > 0:
                    logger.info(f"✅ Confirmed {ul_data['liCount']} li elements in UL {ul_index + 1}")
                    logger.debug(f"🔍 UL {ul_index + 1} content preview: {ul_data['textContent']}...")
                    ...
                    logger.info(f"✅ Successfully extracted {elements_added} elements from UL {ul_index + 1}")
        ...
        for i, element in enumerate(elements):
            try:
                ...
                if flight_data:
                    ...
                    logger.info(f"✅ Extracted flight {i+1}: {flight_data.price} - "
    ...
    async def _extract_airline_robust(self, element: ElementHandle) -> str:
        ...
        try:
            all_text_elements = await element.query_selector_all('span, div')
            airline_patterns = ['United', 'American', 'Delta', 'Southwest', 'JetBlue', 
                              'Alaska', 'Spirit', 'Frontier', 'Lufthansa', 'British Airways']
    ...
    async def _extract_times_robust(self, element: ElementHandle) -> Tuple[str, str]:
        ...
        for selector in semantic_selectors:
            try:
                ...
                if len(time_elements) >= 2:
                    ...
                    logger.debug(f"✅ Found times via semantic selector {selector}: "
        ...
        for selector in class_selectors:
            try:
                ...
                if len(time_elements) >= 2:
                    departure_time = await time_elements[0].inner_text()
                    arrival_time = await time_elements[-1].inner_text()
                    logger.debug(f"✅ Found times via class selector {selector}: "
                               f"{departure_time} -> {arrival_time}")
                    return departure_time.strip(), arrival_time.strip()
            except:
                continue
        ...
        try:
            ...
            if len(times_found) >= 2:
                ...
                logger.debug(f"✅ Found times via pattern matching: "
                           f"{departure_time} -> {arrival_time}")
                return departure_time.strip(), arrival_time.strip()
