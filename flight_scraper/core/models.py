...
class SelectorAttempt(BaseModel):
    ...
    failure_type: Optional[SelectorFailureType] = Field(
        None, description="Type of failure if unsuccessful"
    )
    ...
    attempted_at: datetime = Field(
        default_factory=datetime.now, description="When this attempt occurred"
    )
...
class SelectorMonitoring(BaseModel):
    ...
    element_type: str = Field(
        ..., description="Type of element being selected (e.g., 'origin_input', 'search_button')"
    )
    attempts: List[SelectorAttempt] = Field(
        default_factory=list, description="All selector attempts made"
    )
    successful_selector: Optional[str] = Field(
        None, description="The selector that ultimately succeeded"
    )
    ...
    final_success: bool = Field(
        default=False, description="Whether any selector ultimately succeeded"
    )
...
class PageSelectorHealth(BaseModel):
    ...
    selector_monitoring: Dict[str, SelectorMonitoring] = Field(
        default_factory=dict, description="Monitoring for each element type"
    )
    overall_success_rate: float = Field(
        default=0.0, description="Overall success rate across all selectors"
    )
    critical_failures: List[str] = Field(
        default_factory=list, description="Critical selector failures"
    )
    page_structure_changed: bool = Field(
        default=False, description="Whether page structure appears to have changed"
    )
    scraping_timestamp: datetime = Field(
        default_factory=datetime.now, description="When this page was scraped"
    )
...
class SelectorFailureAlert(BaseModel):
    ...
    dom_changes_detected: bool = Field(
        default=False, description="Whether DOM changes were detected"
    )
